		...
		<div id="list-servicio" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="numero" title="${message(code: 'servicio.numero.label', default: 'Numero')}" />
					
						<g:sortableColumn property="periodo" title="${message(code: 'servicio.periodo.label', default: 'Periodo')}" />
				...
				<g:each in="${servicioInstanceList}" status="i" var="servicioInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${servicioInstance.id}">${fieldValue(bean: servicioInstance, field: "numero")}</g:link></td>
					
						<td><g:formatDate date="${servicioInstance.periodo}" /></td>
